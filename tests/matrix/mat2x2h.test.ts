...
import { Mat2x2H } from "../../src/matrix/mat2x2h.ts";

import * as memory from "../../src/mod.ts";

const TestOffset = 16;

const TypeName = "Mat2x2H";

const M = new Mat2x2H();

const TypedArray = Float16Array;
...
const MatrixByteSize = 8;

const MatrixAlignment = 4;

const MatrixArrayStride = 8;

const MatrixColumnCount = 2;

const MatrixRowCount = 2;

const ComponentTypeSize = 2;

function read(buffer: ArrayBuffer, offset: number): number {
  return new DataView(buffer).getFloat16(offset, true);
}
...
Deno.test(`${TypeName}.componentType`, () => {
  assertEquals(M.componentType, memory.Float16);
});

Deno.test(`${TypeName}.toString`, () => {
  assertEquals(String(M), "mat2x2<f16>");
});

Deno.test(`${TypeName}.toCode`, () => {
  assertEquals(M.toCode("memory"), "memory.Mat2x2H");
});
