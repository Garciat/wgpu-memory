...
import { Mat4x4H } from "../../src/matrix/mat4x4h.ts";

import * as memory from "../../src/mod.ts";

const TestOffset = 16;

const TypeName = "Mat4x4H";
...
const MatrixByteSize = 32;

const MatrixAlignment = 8;

const MatrixArrayStride = 32;
...
Deno.test(`${TypeName}.toString`, () => {
  assertEquals(String(M), "mat4x4<f16>");
});

Deno.test(`${TypeName}.toCode`, () => {
  assertEquals(M.toCode("memory"), "memory.Mat4x4H");
});
