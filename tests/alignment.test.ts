...
import { alignOfVecN, sizeOfVecN } from "../src/internal/alignment.ts";
 ...
 */
Deno.test("WGSL alignment examples", () => {
...
Deno.test("unsupported alignment cases", () => {
  ...
  assertThrows(() => alignOfVecN(4, asAny(memory.Vec4F)), TypeError);
  assertThrows(() => alignOfVecN(asAny(5), memory.Float32.type), RangeError);

  assertThrows(() => sizeOfVecN(2, asAny(memory.Vec2I)), TypeError);
  assertThrows(() => sizeOfVecN(3, asAny(memory.Vec3I)), TypeError);
  assertThrows(() => sizeOfVecN(4, asAny(memory.Vec4I)), TypeError);
  assertThrows(() => sizeOfVecN(asAny(5), memory.Float32.type), RangeError);
});
